export class SoundController {
    ...
    playSound(soundType) {
        ...
        if (sound) {
            sound.currentTime = 0;
            sound.play().catch((err) => console.warn("Audio playback failed (usually requires user interaction first):", err));
        }
    }
    playBackGroundAudio() {
        if (this.backAudio) {
            this.backAudio.currentTime = 0;
            this.backAudio.play().catch((err) => console.warn("Audio playback failed :", err));
        }
    }
    stopAudio() {
        if (this.backAudio) {
